...
```{r load-data}
unvotes <- readRDS("data/unvotes.rds")
```
...
`r params$country1` changed over time on a variety of issues, and compares it 
to two other countries: `r params$country2` and `r params$country3`.
...
```{r plot-yearly-yes-issue, fig.width=10, fig.height=6}
unvotes %>%
  filter(country %in% c(params$country1, params$country2, params$country3)) %>%
